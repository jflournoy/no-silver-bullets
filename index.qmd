...
flextable(ct_model_pars_fe) |>
    ...
    add_footer_lines('Posterior Median: Median of the posterior distribution, used as point estimate. 95% HDI: 95% Highest Density Interval, containing the most probable parameter values with 95% posterior probability mass. Sign Probability: Probability that the effect is in the reported direction, calculated as the proportion of posterior samples with the same sign as the point estimate.')

```

```{r}
#| label: fig-quarter
#| fig-cap: "Effect of within-quarter month. Background hexagons represent density of data, with darker colors indicating greater density. Lines are median posterior expectations, with 95% credible interval ribbons."
#| fig-scap: "Within-quarter month effect on cycle time"
#| fig-alt: "Effect of within-quarter month on cycle time"


knitr::include_graphics("plots/cycle_time_full_intx_lin_remonth_quarter.png", dpi = 300)
```

```{r}
#| label: fig-month
#| fig-cap: "Effect of month. Background hexagons represent density of data, with darker colors indicating greater density. Lines are median posterior expectations, with 95% credible interval ribbons."
#| fig-scap: "Month effect on cycle time"
#| fig-alt: "Effect of month on cycle time"

knitr::include_graphics("plots/cycle_time_full_intx_lin_remonth_month_num.png", dpi = 300)
```
...
```{r}
#| label: fig-codingdays
#| fig-cap: "Effect of average coding days per week on cycle time. Background hexagons represent density of data, with darker colors indicating greater density. Lines are median posterior expectations, with 95% credible interval ribbons."
#| fig-scap: "Coding days per week effect on cycle time"
#| fig-alt: "Effect of average coding days per week on cycle time"

knitr::include_graphics("plots/cycle_time_full_intx_lin_remonth_coding_days.png", dpi = 300)
```
...
```{r}
#| label: fig-mergedprs
#| fig-cap: "Effect of merged PRs on cycle time. Background hexagons represent density of data, with darker colors indicating greater density. Lines are median posterior expectations, with 95% credible interval ribbons."
#| fig-scap: "Merged PRs effect on cycle time"
#| fig-alt: "Effect of merged PRs on cycle time"

knitr::include_graphics("plots/cycle_time_full_intx_lin_remonth_merged_prs.png", dpi = 300)
```
...
```{r}
#| label: fig-defecttickets
#| fig-cap: "Effect of percent defect tickets on cycle time. Background hexagons represent density of data, with darker colors indicating greater density. Lines are median posterior expectations, with 95% credible interval ribbons."
#| fig-scap: "Defect tickets effect on cycle time"
#| fig-alt: "Effect of percent defect tickets on cycle time"

knitr::include_graphics("plots/cycle_time_full_intx_lin_remonth_defect_tickets.png", dpi = 300)
```
...
```{r}
#| label: fig-degree
#| fig-cap: "Effect of degree centrality on cycle time. Background hexagons represent density of data, with darker colors indicating greater density. Lines are median posterior expectations, with 95% credible interval ribbons."
#| fig-scap: "Degree centrality effect on cycle time"
#| fig-alt: "Effect of degree centrality on cycle time"

knitr::include_graphics("plots/cycle_time_full_intx_lin_remonth_degree_cent.png", dpi = 300)
```
...
```{r}
#| label: fig-comments
#| fig-cap: "Effect of comments per PR on cycle time. Background hexagons represent density of data, with darker colors indicating greater density. Lines are median posterior expectations, with 95% credible interval ribbons."
#| fig-scap: "Comments per PR effect on cycle time"
#| fig-alt: "Effect of comments per PR on cycle time"

knitr::include_graphics("plots/cycle_time_full_intx_lin_remonth_pr_comments.png", dpi = 300)
```
...
```{r}
#| fig-cap: "Expected change in cycle time from 50th to 90th percentile of each variable, by organization and month. The color represents the expected change in cycle time, with warm indicating an increase in cycle time and cool indicating a decrease. The scale is the same across all plots."
#| fig-scap: "Expected change in cycle time from 50th to 90th percentile of each variable"
#| fig-alt: "Expected change in cycle time from 50th to 90th percentile of each variable, by organization and month"
#| fig-subcap: 
#|   - "Avg. coding days per Week"
#|   - "Total merged PRs"
#|   - "Defect tickets percentage"
#|   - "Degree centrality"
#|   - "Comments per PR"
#| label: fig-heatmaps
#| layout: [[333,333,333],[-166,333,333,-166]]

knitr::include_graphics("plots/avg_coding_days_per_week_eff_size.png", dpi = 300)
knitr::include_graphics("plots/total_merged_prs_eff_size.png", dpi = 300)
knitr::include_graphics("plots/defect_tickets_pct_indiv_eff_size.png", dpi = 300)
knitr::include_graphics("plots/degree_centrality_monthly_100_eff_size.png", dpi = 300)
knitr::include_graphics("plots/comments_per_pr_eff_size.png", dpi = 300)
...
# Fix the NA values for Parameter column - if it has Month as predictor and is SD, it's likely λ
...
ft <- flextable(display_dt) |>
  ...
  add_footer_lines( 
  'Par: Response distribution parameter (λ: rate parameter, k: shape parameter). Grouping: Random effects grouping structure. Stat: Parameter type (SD: standard deviation of random effect, Correlation: correlation between random effects). Predictor: Fixed or random effect term. Posterior Median: Median of the posterior distribution, used as point estimate. 95% HDI: 95% Highest Density Interval, containing the most probable parameter values with 95% posterior probability mass. Sign Probability: Probability that the effect is in the reported direction, calculated as the proportion of posterior samples with the same sign as the point estimate.')

ft
```

```{r}
#| fig-cap: "Posterior prediction densities for model-expected distributions of cycle time across all orgs with at least 10 observations. Each row is one organization, showing the density of predicted cycle times from 50 model simulations. The scale has been transformed to better show the spread of data. Columns have been ordered by sample size, and rows are ordered by median predicted cycle time. Density fill colors reflect sample size. Note that distributions from larger orgs have less variability in their posterior predictions. Line at 4 weeks is set arbitrarily to allow comparisons"
#| fig-alt: "Cycle Time Variability Across Organizations"
#| fig-scap: "Cycle Time Variability Across Organizations"
#| label: fig-pp-check-org

knitr::include_graphics("plots/cycle_time_full_intx_nlq_pp_check.png", dpi = 300)

```
```{r}
#| fig-cap: "Posterior Prediction Distributions of Cycle Time for all data"
#| fig-alt: "Posterior Prediction Distributions of Cycle Time for all data"
#| label: fig-pp-check-sum

knitr::include_graphics("plots/cycle_time_full_intx_nlq_pp_check_summary.png", dpi = 300)
...
```{r}
#| fig-cap: "Between-person effects on cycle-time trajectories across the year for randomly-sampled individuals."
#| fig-scap: "Between-person effects on cycle-time trajectories"
#| fit-alt: "Between-person effects on cycle-time trajectories across the year for randomly-sampled individuals."
#| label: fig-bw-ct-traj

knitr::include_graphics("plots/cycle_time_full_intx_lin_remonth_post_pred_ids.png", dpi = 300)
```
...
```{r}
#| fig-cap: "Within-person effects on cycle-time trajectories across the year for randomly-sampled individuals."
#| fig-scap: "Within-person effects on cycle-time trajectories"
#| fig-alt: "Within-person effects on cycle-time trajectories across the year for randomly-sampled individuals."
#| label: fig-wi-ct-traj

knitr::include_graphics("plots/cycle_time_full_intx_nlq_post_pred_ids_win.png", dpi = 300)
