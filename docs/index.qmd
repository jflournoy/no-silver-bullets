...
::: {#fig-quarter fig-scap="Within-quarter month doesn't affect cycle time" fig-alt="Within-quarter month doesn't affect cycle time" dpi=300}

![](plots/cycle_time_full_intx_lin_remonth_quarter.png)

Within-quarter month doesn't affect cycle time. Background pixels represent density of data, with darker colors indicating greater density. Lines are median posterior expectations, with 95% credible interval ribbons.

:::


::: {#fig-month fig-scap="Slight reduction of cycle time across the year" fig-alt="Slight reduction of cycle time across the year"}

![](plots/cycle_time_full_intx_lin_remonth_month_num.png)

Slight reduction of cycle time across the year. Background hexagons represent density of data, with darker colors indicating greater density. Lines are median posterior expectations, with 95% credible interval ribbons.
...
::: {#fig-codingdays fig-scap="More coding days is associated with shorter cycle times" fig-alt="More coding days is associated with shorter cycle times"}
...
::: {#fig-mergedprs fig-scap="More merged PRs is associated with shorter cycle times" fig-alt="More merged PRs is associated with shorter cycle times"}
...
::: {#fig-degree fig-scap="Higher degree centrality is associated with shorter cycle times" fig-alt="Higher degree centrality is associated with shorter cycle times"}
...
::: {#fig-comments fig-scap="More comments per PR is associated with longer cycle times" fig-alt="More comments per PR is associated with longer cycle times"}
...
::: {#fig-heatmaps layout="[[333,333,333],[-166,333,333,-166]]" fig-scap="Effect sizes for each variable are heterogeneous across time, organizations, and values of other predictors" fig-alt="Effect sizes for each variable are heterogeneous across time, organizations, and values of other predictors"}

![](plots/avg_coding_days_per_week_eff_size.png){#fig-heatmaps-coding}



![Avg. coding days per Week](plots/total_merged_prs_eff_size.png){#fig-heatmaps-prs}



![Total merged PRs](plots/defect_tickets_pct_indiv_eff_size.png){#fig-heatmaps-defects}



![Defect tickets percentage](plots/degree_centrality_monthly_100_eff_size.png){#fig-heatmaps-centrality}



![Degree centrality](plots/comments_per_pr_eff_size.png){#fig-heatmaps-comments}

Comments per PR

Effect sizes for each variable are heterogeneous across time, organizations, and values of other predictors. These plots show the xpected change in cycle time from 50th to 90th percentile of each variable, by organization and month. The color represents the expected change in cycle time, with warm indicating an increase in cycle time and cool indicating a decrease. The scale is the same across all plots.
:::
